...
cv::Mat Processor::process_image(cv::Mat& message)
{
    ...
    cv::cvtColor(resized_image, grayscale_image, cv::COLOR_BGR2GRAY);//Converting BGR to Grayscale image and storing it into 'converted' matrix//
    cv::threshold(grayscale_image, binary_image, 100, 255, cv::THRESH_BINARY);//converting grayscale image stored in 'converted' matrix into binary image//
